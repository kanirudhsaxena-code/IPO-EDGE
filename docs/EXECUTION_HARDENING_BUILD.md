...
Owner: ChatGPT Executor
Target: Cycles 3–5
- [ ] Add live canonical identity map using ISIN first, then exchange identifier / Upstox IPO ID, then canonical company name + aliases. Resolver core committed in `scripts/execution_hardening_identity.py`; hardening-only shadow ingress committed in `scripts/execution_hardening_identity_ingress.py` with fail-closed feature/master-switch and no-production-write guards in `tests/test_execution_hardening_identity_ingress.py`. Actual live/shadow canonical-map feed remains pending, so this item stays unchecked.
