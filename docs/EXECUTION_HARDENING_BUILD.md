...
Owner: ChatGPT Executor
Target: Cycles 3–5
- [ ] Add live canonical identity map using ISIN first, then exchange identifier / Upstox IPO ID, then canonical company name + aliases. Resolver core committed in `scripts/execution_hardening_identity.py`; hardening-only shadow ingress committed in `scripts/execution_hardening_identity_ingress.py`; additive canonical-map feed builder committed in `scripts/execution_hardening_identity_feed.py` with fail-closed conflict/ambiguity guards in `tests/test_execution_hardening_identity_feed.py`. Actual fetched live/shadow provider feed evidence remains pending, so this item stays unchecked.
