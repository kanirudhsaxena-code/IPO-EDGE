...
def log_run(conn, run_type: str, framework_version: str = "1.0") -> int:
    row = conn.execute(
        """
        INSERT INTO run_log (run_type, framework_version, status)
...
def upsert_ipo(conn, ipo: dict) -> int:
    row = conn.execute(
        """
        INSERT INTO ipos (
          company_name, segment, exchange, issue_open_date, issue_close_date,
          listing_date, price_band_low, price_band_high, issue_price, issue_size, status
        ) VALUES (
          %(company_name)s, %(segment)s, %(exchange)s, %(issue_open_date)s, %(issue_close_date)s,
...
def insert_checkpoint(conn, ipo_id: int, checkpoint: dict) -> int:
    row = conn.execute(
        ...
        INSERT INTO checkpoints (
          ipo_id, checkpoint_type, checkpoint_time, score, grade, decision,
          bear_gain_estimate, base_gain_estimate, bull_gain_estimate, confidence,
          hard_blocker, evidence_delta_summary, framework_version
        ) VALUES (
          %(ipo_id)s, %(checkpoint_type)s, %(checkpoint_time)s, %(score)s, %(grade)s, %(decision)s,
